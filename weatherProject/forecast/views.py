...
import requests # this library helps us to fech data from API
import pandas as pd # for handling and analysing data
import numpy as np # for numerical operations
from sklearn.model_selection import train_test_split # to split data into training and testing sets
from sklearn.preprocessing import LabelEncoder #to conveert catogerical data into numericals value
...
def prepare_data(data):
  ...
  X = data[['MinTemp', 'MaxTemp', 'WindGustDir','WindGustSpeed','Humidity','Pressure','Temp']] #feture variable
...
def prepare_regression_data(data, feature):
  X , y = [],[] # instialize the list for feature and target values
...
def weather_view(request):
  if request.method == 'POST':
    ...
    future_humidity = predict_future(hum_model, current_weather['humidity'])  # Fixed: hum_model
    
    # Prepare time for future predictions
    timezone = pytz.timezone('Asia/Kolkata')
    now = datetime.now(timezone)
    next_hour = now + timedelta(hours=1)
    next_hour = next_hour.replace(minute=0, second=0, microsecond=0)  # Fixed: minute (singular)
